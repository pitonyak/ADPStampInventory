...
int main(int argc, char **argv){
  ...
  if (create_anomaly_list) {
    std::unordered_set<std::string>* ips = read_text_file(out_ip_fname);
    std::unordered_set<std::string>* macs = read_text_file(out_mac_fname);
    if (ips != nullptr && macs != nullptr && eth_type_set != nullptr) {
