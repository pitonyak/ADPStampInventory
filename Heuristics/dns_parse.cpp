...
#include "dns_tcp.h"
#include "dns_rtypes.h"
#include "dns_strutils.h"

// If you want a reasonable place to start walking through the code, 
// go to the 'handler' function.
                    
std::string DEFAULT_TCP_STATE_PATH = "/tmp/dnsparse_tcp.state";
...
void print_rr_section(dns_rr *, const char *, config *);
...
size_t smart_strlen(const char* c) {
    if (c == nullptr) {
        std::cout << "smar_strlen with a nullptr" << std::endl;
    }
    return c == nullptr ? 0 : smart_strlen(c);
}
...
int main(int argc, char **argv) {
    ...
    while (c != -1) {
        switch (c) {
            ...
            case 'f':
                ...
                return 0;
            case 'm':
                conf.RECORD_SEP = (optarg == nullptr) ? "" : optarg;
                conf.SEP = '\n';
                break;
            case 'M':
            ...
            case 'r':
                ...
                break;
            case 's':
                conf.TCP_STATE_PATH = (optarg == nullptr) ? "" : optarg;;
                break;
            case 'S':
                TCP_SAVE_STATE = 0;
                break;
            case 't':
                ...
                break;
            case 'D':
                conf.DEDUPS = strtoul(optarg, nullptr, 10);
    ...
    if (arg_failure) {
        ...
        DEFAULT_TCP_STATE_PATH.c_str());
        return -1;
    }

    conf.ip_fragment_head = nullptr;

    // Load and prior TCP session info
    conf.tcp_sessions_head = nullptr; 
    if (TCP_SAVE_STATE == 1) {
        tcp_load_state(&conf);
    }

    //conf.dedup_hashes = calloc(conf.DEDUPS, sizeof(uint64_t));
    conf.dedup_hashes = new unsigned long long[conf.DEDUPS];
    for (uint32_t idx=0; idx < conf.DEDUPS; ++idx) {
        conf.dedup_hashes[idx] = 0;
    }
    ...
    if (TCP_SAVE_STATE == 1) {
        ...
        tcp_expire(&conf, nullptr);
    }

    delete[] conf.dedup_hashes;
    ip_frag_free(&conf);
...
void handler(uint8_t * args, const struct pcap_pkthdr *orig_header, 
    ...
    // value, so they set the packet pointer to nullptr on failure.
...
void print_summary(ip_info * ip, transport_info * trns, dns_info * dns,
    ...
    const char* null_parser_name = "nullptr";
    qnext = dns->queries;
    while (qnext != nullptr) {
        printf("%c? ", conf->SEP);
        if (conf->PRINT_RR_NAME) {
            ...
            // Original code complained that parser->name is nullptr and used
            // in the printf.
            if (parser->name == nullptr) 
                printf("%s UNKNOWN(%s,%d)", qnext->name, null_parser_name, qnext->type);
            else 
    ...
    printf("%c%s\n", conf->SEP, conf->RECORD_SEP.c_str());
...
void print_rr_section(dns_rr * next, const char * name, config * conf) {
    int skip;
    int i;
    while (next != nullptr) {
        ...
        if (!skip) {
            ...
            name = (next->name == nullptr) ? "*empty*" : next->name;
            data = (next->data == nullptr) ? "*empty*" : next->data;
            if (conf->PRINT_RR_NAME) { 
                if (next->rr_name == nullptr) 
                    // Handle bad records.
...
void dns_rr_free(dns_rr * rr) {
    ...
    if (rr->name != nullptr) delete[] rr->name;
    if (rr->data != nullptr) delete[] rr->data;
    dns_rr_free(rr->next);
    delete rr;
}
...
void dns_question_free(dns_question * question) {
    ...
    if (question->name != nullptr) delete[] question->name;
...
uint32_t parse_questions(uint32_t pos, uint32_t id_pos, 
    ...
    dns_question * last = nullptr;
    dns_question * current;
    uint16_t i;
    *root = nullptr;

    for (i=0; i < count; i++) {
        current = new dns_question();
        current->next = nullptr; current->name = nullptr;

        current->name = read_rr_name(packet, &pos, id_pos, header->len);
        if (current->name == nullptr || (pos + 2) >= header->len) {
            ...
            current->name = new char[smart_strlen(buffer) + smart_strlen(msg) + 1];
            // The compiler incorrectly assumes that current->name may be 
            // null and ignores that we just allocated space for it!
            if (current->name != nullptr)
                sprintf(current->name, "%s%s", msg, buffer);
            delete[] buffer;
            current->type = 0;
            current->cls = 0;
            if (last == nullptr) *root = current;
...
uint32_t parse_rr(uint32_t pos, uint32_t id_pos, struct pcap_pkthdr *header, 
    ...
    rr_parser_container opts_cont = {0, 0, &opts, nullptr, nullptr, 0};
    ...
    if (rr->name == nullptr) {
        ...
        rr->name = new char[smart_strlen(msg) + 1];
    ...
    if (conf->MISSING_TYPE_WARNINGS && get_default_rr_parser() == parser) 
    ...
    if (header->len < (rr_start + 10 + rr->rdlength)) {
        ...
        char * buffer = new char[smart_strlen(rr->data) + smart_strlen(msg) + 1];
...
uint32_t parse_rr_set(uint32_t pos, uint32_t id_pos, 
    ...
    *root = nullptr; 
    for (i=0; i < count; i++) {
        ...
        current = new dns_rr();
        current->next = nullptr; current->name = nullptr; current->data = nullptr;
