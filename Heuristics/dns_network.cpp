...
// Moved here from network.h because the linker
// complains that it is defined in every file
// that includes network.h.
// January 16, 2023
...
// which means zero is a valid return value. Sets p_packet to nullptr on error.
// See RFC791
uint32_t ipv4_parse(uint32_t pos, struct pcap_pkthdr *header, 
                    uint8_t ** p_packet, ip_info * ip, config * conf) {

    uint32_t h_len;
    ip_fragment * frag = nullptr;
    uint8_t frag_mf;
    ...
    if (frag_mf == 1 || frag_offset != 0) {
        ...
        frag->data = new uint8_t[ip->length];
        frag->next = frag->child = nullptr;
...
//                mean lost data.
ip_fragment * ip_frag_add(ip_fragment * this_frag, config * conf) {
    ip_fragment ** curr = &(conf->ip_fragment_head);
    ip_fragment ** found = nullptr;

    DBG(printf("Adding fragment at %p\n", this_frag);)

    // Find the matching fragment list.
    while (*curr != nullptr) {
        if ((*curr)->id == this_frag->id && 
            IP_CMP((*curr)->src, this_frag->src) &&
            IP_CMP((*curr)->dst, this_frag->dst)) {
            found = curr;
    ...
    // 'found' could also be nullptr, meaning no match was found.

    // If there wasn't a matching list, then we're done.
    if (found == nullptr) {
        DBG(printf("No matching fragments.\n");)
        this_frag->next = conf->ip_fragment_head;
        conf->ip_fragment_head = this_frag;
        return nullptr;
    }
    ...
    while (*found != nullptr) {
        DBG(printf("*found: %u-%u, this: %u-%u\n",
                   ...
                   this_frag->start, this_frag->end);)
        if ((*found)->start >= this_frag->end) {
            DBG(printf("It goes in front of %p\n", *found);)
            // It goes before, so put it there.
            this_frag->child = *found;
            this_frag->next = (*found)->next;
            *found = this_frag;
            break;
        } else if ((*found)->child == nullptr && 
                    (*found)->end <= this_frag->start) {
            DBG(printf("It goes at the end. %p\n", *found);)
           // We've reached the end of the line, and that's where it
            // goes, so put it there.
            (*found)->child = this_frag;
            break;
        }
        DBG(printf("What: %p\n", *found);)
        found = &((*found)->child);
    ...
    found = curr;
    while ((*found != nullptr) && (*found)->child != nullptr) {
        ...
        if ((*found)->end == child->start) {
            ...
            uint8_t * buff = new uint8_t[fnd_len + child_len];
...
void ip_frag_free(config * conf) {
    ...
    while (conf->ip_fragment_head != nullptr) {
...
uint32_t udp_parse(uint32_t pos, struct pcap_pkthdr *header, 
                   ...
                   __attribute__ ((unused)) config * conf) {
    if (header->len - pos < 8) {
