...
int process_pcap_ospf(MacAddresses& dest_mac_to_ignore, MacAddresses& mac_addresses, IpAddresses& ip_addresses, const EthernetTypes& ethernet_types, const IPTypes& ip_types, const std::string& pcap_filename, std::string output_directory, std::string extra_heuristic_name, bool verbose, bool generateCSV, std::atomic_bool* abort_requested) {
  ...
  while(!done && (abort_requested == nullptr || !abort_requested->load()) ){
    ...
    if (ether_type_int == ETHERTYPE_IP) {
      ...
      // ????????
      // This code is not used or tested by anyone, it exists to demonstate how
      // Andrew Pitonyak thinks you should find and process OSPF using libpcap
      // This code is taken from the heuristic code and it is trying to do the heuristic
      // so all of that must be removed!
      // OSPFIGP
      if (ip_p == 89) {
        // OSPF Traffic found!
        const char* ospf_types[] = {"(0) Invalid", "(1) Hello", "(2) DD-Database Description", "(3) LSR-Link State Request", "(4) LSU-Link State Update", "(5) LSAck-Link State Acknowledgement", "(>5) Invalid"};
        const char* ospf_autype[] = {"(0) None", "(1) Plaintext", "(2) MDF", "(>2) Invalid"};

        const uint8_t* ospf_header_start = (const uint8_t*)(pkt_data + offset_to_data_ipv4);

        // Version should be 2 for IPv4
        int ospf_version = *ospf_header_start;
        int ospf_type = *(ospf_header_start + 1);

        // combined length of the OSPF header and the OSPF data portion.
        uint16_t ospf_len = ntohs(*(uint16_t*)(ospf_header_start + 2));
        uint32_t ospf_router_id = ntohl(*(const uint32_t*)(ospf_header_start + 4));
        uint32_t ospf_area_id = ntohl(*(const uint32_t*)(ospf_header_start + 8));
        //int ospf_checksum = ntohs(*(uint16_t*)(ospf_header_start + 12));
        int ospf_auth_type = ntohs(*(uint16_t*)(ospf_header_start + 14));

        int ospf_type_idx = ospf_type < 7 ? ospf_type : 6;
        int ospf_autype_idx = ospf_auth_type < 3 ? ospf_auth_type : 3;

        std::cout << "OSPF at " << it << " version:" << ospf_version << " type:" << ospf_type << "=" << ospf_types[ospf_type_idx]
                  << " len:" << ospf_len << " router_id:" << ospf_router_id << " area_id:" << ospf_area_id 
                  << " auth_type:" << ospf_auth_type << "=" << ospf_autype[ospf_autype_idx] << std::endl;

      } else if (ip_p == static_cast<int>(IPPROTO_TCP)) {
