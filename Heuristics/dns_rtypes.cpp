...
// Note: The nullptr in the name field is important, and used to distinguish
//       this from the rest of the parser containers.
rr_parser_container default_rr_parser = {0, 0, escape, nullptr, nullptr, 0};

// January 16, 2023
// Add this because of link errors for multiple definitions of
// the default parser. 
rr_parser_container * get_default_rr_parser() {
...
char * A(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t i,
         uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
...
char * soa(const uint8_t * packet, uint32_t pos, uint32_t id_pos,
    ...
    if (mname == nullptr) return mk_error("Bad SOA: ", packet, pos, rdlength);
    rname = read_rr_name(packet, &pos, id_pos, plen);
    if (rname == nullptr) return mk_error("Bad SOA: ", packet, pos, rdlength);
...
char * mx(const uint8_t * packet, uint32_t pos, uint32_t id_pos,
    ...
    buffer = new char[5 + 1 + strlen(name) + 1];
...
char * opts(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t id_pos,
                  uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
    ...
    buffer = new char[strlen(base_format) - 20 + 5 + 8 + strlen(rdata) + 1]; 
...
char * srv(const uint8_t * packet, uint32_t pos, uint32_t id_pos,
    ...
    buffer = new char[(3*5+3) + strlen(target)];
...
char * AAAA(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t id_pos,
                  uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
    char *buffer;
    uint16_t ipv6[8];
    int i;
...
char * dnskey(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t id_pos,
                    uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
    ...
    buffer = new char[1 + strlen(key) + 18];
    if (buffer != nullptr)
...
char * rrsig(const uint8_t * packet, uint32_t pos, uint32_t id_pos,
    ...
    buffer = new char[(2*5 + // 2 16 bit ints
                                    3*10 + // 3 32 bit ints
                                    2*3 + // 2 8 bit ints
                                    8 + // 8 separator chars
                                    strlen(signer) +
...
char * nsec(const uint8_t * packet, uint32_t pos, uint32_t id_pos,
    ...
    buffer = new char[strlen(domain)+strlen(bitmap)+2];
    if (buffer != nullptr)
...
char * ds(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t id_pos,
                          uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
    ...
    buffer = new char[strlen(digest) + 15];
    if (buffer != nullptr)
...
#define nullptr_DOC "This data is simply hex escaped. \n"\
"Non printable characters are given as a hex value (\\x30), for example."
char * escape(const uint8_t * packet, uint32_t pos, __attribute__ ((unused)) uint32_t i,
              uint16_t rdlength, __attribute__ ((unused)) uint32_t plen) {
...
rr_parser_container rr_parsers[] = {{1, 1, A, "A", A_DOC, 0},
                                    ...
                                    {0, 16, escape, "TEXT", nullptr_DOC, 0}, 
                                    {0, 47, nsec, "NSEC", NSEC_DOC, 0},
                                    {0, 43, ds, "DS", DS_DOC, 0},
                                    {0, 10, escape, "nullptr", nullptr_DOC, 0}, 
                                    {0, 48, dnskey, "DNSKEY", KEY_DOC, 0},
                                    {0, 255, escape, "ANY", nullptr_DOC, 0}
                                   };
...
rr_parser_container * find_parser(uint16_t cls, uint16_t rtype) {
    ...
    rr_parser_container * found = nullptr;
