...
void tcp_parse(uint32_t pos, struct pcap_pkthdr *header, 
    ...
    tcp->next_sess = nullptr;
    tcp->next_pkt = nullptr;
    tcp->prev_pkt = nullptr;
    tcp->ts = header->ts;
    tcp->src = ip->src;
    tcp->dst = ip->dst;
...
void tcp_expire(config * conf, const struct timeval * now ) {
    ...
    while (head != nullptr) {
        ...
        if (offset_found == 0) {
            if   (head->len > 2 && 
                  (uint32_t)TCP_DNS_LEN(head->data, 0) < head->len &&
...
tcp_info * tcp_assemble(tcp_info * base) {
    ...
    for (curr=&base; *curr != nullptr; curr = &(*curr)->prev_pkt) {
        dc_i++;
        base = *curr;
    }
    ...
    data_chain = new char*[dc_i];
    data_lengths = new uint32_t[dc_i];

    for (size_t idx=0; idx<dc_i; ++idx) {
        data_lengths[idx] = 0;
        data_chain[idx] = nullptr;
    }
    ...
    // Skip all this if the origin is nullptr, since we don't have a starting
    // point anyway.
    dc_i = 0;
    while (base != nullptr && origin != nullptr) {
        // Search for the packet with the next sequence number that has 
        // non-zero length.
        tcp_info ** next_best = nullptr;
        for (curr = &base; *curr != nullptr; curr = &(*curr)->next_pkt) {
            if ((*curr)->sequence == curr_seq) {
                if ((*curr)->len > 0) {
                    // We found a packet at that sequence with data, it 
                    // should be what we want.
                    break;
                } else if (next_best == nullptr) {
        ...
        if (*curr == nullptr && next_best != nullptr) {
        ...
        if (*curr != nullptr && (origin->sequence == (*curr)->sequence)) {
            origin = *curr;
        }
    ...
    if (total_length > 0) {
        final_data = new uint8_t[total_length];
        for(i=0; i < dc_i; i++) {
            if (data_chain[i] != nullptr) { 
...
// set to nullptr. The prev_pkt pointer will be 1 if there is another
// packet in the session after this one. 
void tcp_save_state(config * conf) {
    FILE * outfile = fopen(conf->TCP_STATE_PATH.c_str(),"w");
    tcp_info * next = conf->tcp_sessions_head;
    tcp_info * curr_pkt;
    ...
    while (next != nullptr) {
        ...
        while (curr_pkt != nullptr) {
            ...
            curr_pkt->next_sess = nullptr;
            curr_pkt->next_pkt = nullptr;
            //
            // TODO: ????
            // What is this? I hate this in that we set the previous
            // packet to (NULL+1), which should be a 1. Nope, I do not like it.
            // I do NOT like this logic / code.
            // All we need to know is whether there is a prev. packet.
            //curr_pkt->prev_pkt = (prev_pkt == NULL) ? (NULL+1) : NULL;
            if (prev_pkt == nullptr)
                curr_pkt->prev_pkt = (tcp_info*)1;
            else
                curr_pkt->prev_pkt = nullptr;
            curr_pkt->data = nullptr;
...
tcp_info * tcp_load_state(config * conf) {
    FILE * infile;
    struct stat i_stat;
    int ret = stat(conf->TCP_STATE_PATH.c_str(), &i_stat);
    size_t read;
    tcp_info * pkt;
    tcp_info * prev = nullptr;
    tcp_info * first_sess = nullptr;
    ...
    infile = fopen(conf->TCP_STATE_PATH.c_str(), "r");
    if (infile == nullptr) {
    ...
    while (read != 0) {
        ...
        has_prev = (pkt->prev_pkt == nullptr);
        pkt->prev_pkt = nullptr;

        pkt->data = new uint8_t[pkt->len];
